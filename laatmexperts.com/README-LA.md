# Louisiana ATM Experts — www.laatmexperts.com

Generated September 17, 2026 as part of the US ATM Experts 49-state rollout. Same structure, copy, SEO work, and design as the Arizona rebuild (www.azatmexperts.com), localized for Louisiana.

## What's localized for Louisiana

- **Brand**: "Louisiana ATM Experts", positioned as *part of US ATM Experts* (no local-office or founding-year claim).
- **Contact**: 833.843.3331 · info@usatmexperts.com on every page, header, footer, and in the LocalBusiness schema.
- **Colors**: the Louisiana state-flag palette — primary `#1f3f95`, accent `#b8862b`, highlight `#f2c94c` — applied to the CSS and every SVG graphic.
- **Logo**: your existing Louisiana flag puck, shipped in this folder at `_assets/_images/_state_logo/la-puck-logo.png` (header, footer, favicon, Open Graph image, and schema).
- **City pages** (9): New Orleans, Metairie, Baton Rouge, Lafayette, Lake Charles, Shreveport, Monroe, Alexandria, Houma. Each has unique local copy, neighborhoods/districts, nearby-city links, city-specific FAQs, and Service + FAQ + Breadcrumb schema.
...
- **Processing page**: leads with a *Current ATM operators* section (bring your portfolio over: aggressive interchange rates with no numbers stated, no processing fees with daily deposits, painless conversion) plus a floating **Get a quote** button that opens a slide-in form (business name, name, address, number of terminals, monthly transactions). The form posts to Formspree — create a free form at https://formspree.io and replace `YOUR_FORM_ID` in `atm-processing.html` (submissions arrive with the subject "Portfolio processing inquiry (Louisiana ATM Experts)", so one Formspree form can serve every state). Also: every "free processing" mention carries an asterisk with the footnote "Due to additional required compliance, cannabis transactions are subject to a $0.05 processing fee."
...
_assets/_images/*.svg    Hero backgrounds and illustrations, recolored for Louisiana
...
_assets/_images/_state_logo/la-puck-logo.png   State flag puck logo
```
...
1. Upload everything in this folder to the web root of www.laatmexperts.com (usually `public_html/`).
...
1. **Google Search Console** — add `https://www.laatmexperts.com`, submit `sitemap.xml`, request indexing for `index.html`, `atm-placement.html`, `dispensaries.html`, and the top three city pages.
2. **Google Business Profile** — the biggest local-ranking factor. A service-area profile for Louisiana with the 833 number, hours, and photos of installs. Reviews from placed locations matter more than anything on the page.
3. **Citations** — same name / phone / URL on Yelp, BBB, Bing Places, Apple Maps, the Genmega and Hyosung dealer locators, and the Louisiana chamber of commerce.
...
- The "Trusted by Louisiana hospitality & retail businesses" trust bar line assumes you already have (or will have) customers in the state.

## Editing later

`build_state.py` + `states/LA.json` (city data) + `states/palettes.json` (colors) generate every page: `python3 build_all.py LA`. The generated HTML is plain and can also be edited directly — keep one `<h1>` per page and update `sitemap.xml` when adding pages.
